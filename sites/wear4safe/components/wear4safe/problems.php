...
global $nav;
$nav = "Αναφορές προβλημάτων";
$config["navigation"] = $nav;
$item = (intval($_GET["item"]) > 0 ? intval($_GET["item"]) : "");
$BaseUrl = "index.php?com=problems";
					...
					<table class="table table-responsive-lg  table-bordered table-striped mb-0" id="datatable-filters">
						...
						<tbody>
							<?	
								...
								$filter.=($auth->UserType != "Administrator"?' AND t1.employee_id IN (SELECT employee_id FROM employees WHERE organization_id='.$auth->UserRow['organization_id'].")":'');
								$query = "SELECT t1.problem_id,t2.surname,t2.firstname,t3.imei,t3.sensor_name,t4.maptype_name,t2.organization_id,t1.date_insert
								FROM problems t1 INNER JOIN employees t2 ON t1.employee_id=t2.employee_id 
								INNER JOIN sensors t3 ON t1.sensor_id=t3.sensor_id
								INNER JOIN maptypes t4 ON t1.maptype_id = t4.maptype_id
								...
								while ($dr = $db->sql_fetchrow($result))
								{
									?>
										<tr>
											...
											<td><?
											$organizationRow = $db->RowSelectorQuery("SELECT * FROM organizations WHERE organization_id=".$dr['organization_id']);
											echo $organizationRow["organization_name"]?></td>
