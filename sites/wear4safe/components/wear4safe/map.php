...
if( $auth->UserType == "Administrator" )
{
	...
	{	
		if($_REQUEST["Command"] == "SAVE")
		{
			...
			{
				$PrimaryKeys["map_id"] = intval($_GET["item"]);
			...
			$Collector["map_name"] = $_POST["map_name"];
			$QuotFields["map_name"] = true;
			...
			$db->ExecuteUpdater("map",$PrimaryKeys,$Collector,$QuotFields);
			...
			{
				$error=0;
				//Δεν μπορώ να σβήσω ΜΑΠ αν έχω καταχωρημένα maptypes κάτω από αυτό
				$result = $db->sql_query('SELECT * FROM maptypes WHERE map_id='.$item);
				...
				if($error==0) {	
					...
					$filter="";
					$db->sql_query("DELETE FROM map WHERE map_id=" . $item.$filter);
...
if(isset($_GET["item"])) {
	...
	$query="SELECT * FROM map WHERE map_id=".$_GET['item'].$filter;
	$dr_e = $db->RowSelectorQuery($query);
	if (!isset($dr_e["map_id"]) && intval($_GET["item"])> 0) {
		$messages->addMessage("NOT FOUND!!!");
		Redirect("index.php?com=map");
	}
			...
			<section class="card">
				...
				<div class="card-body">
					<div class="form-horizontal form-bordered" method="get">
						...
						<div class="form-group row">
							<label class="col-lg-3 control-label text-lg-right pt-2" for="map_name">Ονομασία</label>
							<div class="col-lg-6">
								<input type="text" class="form-control" id="map_name" name="map_name" value="<?=(isset($dr_e["map_name"]) ? $dr_e["map_name"]:'')?>">
							</div>
						</div>
					...
					<div class="row-fluid" style="margin-top:20px;">
						...
						<? } ?>
						<a href="index.php?com=map"><button type="button" class="btn btn-primary">Επιστροφή</button></a>
					</div>
				</div>
			</section>
		</div>
			...
			<div class="col">
				<section class="card">
					...
					<div class="card-body">
						...
						<!-- <table class="table table-bordered table-striped mb-0" id="datatable-default">-->
							...
							<tbody>
								<?	
									...
									$filter="";
									$query = "SELECT * FROM map WHERE 1=1 ".$filter."  ";
									...
									while ($dr = $db->sql_fetchrow($result))
									{
										?>
											<tr>
												...
												<td><?=$dr["max"]?></td>
												<td>
													<a data-toggle="tooltip" data-placement="top" title="Επεξεργασία" style="padding:4px"  href="index.php?com=map&Command=edit&item=<?=$dr["map_id"]?>"><i style="font-size:24px;" class="fas fa-edit"></i></a>
													<? if($auth->UserType == "Administrator") { ?>
														<a data-toggle="tooltip" data-placement="top" title="Διαγραφή" href="#" onclick="ConfirmDelete('Επιβεβαίωση διαγραφής','index.php?com=map&Command=DELETE&item=<?=$dr["map_id"]?>');"><span><i style="font-size:24px;" class="fas fa-trash"></i> </a></span></a>
													<? } ?>
												</td>
											</tr>
										<?
						...
						<? if($auth->UserType == "Administrator") { ?>
						<div class="row-fluid" style="margin-top:20px;">
							<a href="index.php?com=map&item="><button type="button" class="btn btn-primary">Νέα εγγραφή</button></a>
						</div>
						<? } ?>
					</div>
				</section>
