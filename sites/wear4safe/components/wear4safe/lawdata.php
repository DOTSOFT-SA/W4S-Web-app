		...
		<div class="col">
			<section class="card">
				<header class="card-header">
					...
					<h2 class="card-title">Νομοθετήματα</h2>
				</header>
				<div class="card-body">
					...
					<table class="table table-responsive-lg  table-bordered table-striped mb-0" id="datatable-filters">
						<thead>
							<tr>
								<th style="width:30%;">Επάγγελμα</th>
								<th style="width:20%;">Νομοθέτημα</th>
								<th style="width:30%;">Τύπος μέσου</th>
								<th style="width:10%;">Κατηγορία μέσου</th>
								<th style="width:10%;">Πρότυπα</th>
							</tr>
						</thead>
						<tbody>
							<?	
								...
								$query = "SELECT t1.profession_name,t1.law,t3.maptype_name,t3.standards,t4.map_name 
										FROM professions t1 
										INNER JOIN professionsmap t2 ON t1.profession_id=t2.profession_id
										INNER JOIN maptypes t3 ON t2.maptype_id=t3.maptype_id
										INNER JOIN map t4 ON t3.map_id=t4.map_id 
								WHERE 1=1 ".$filter."  ";
...
<? } else {
	$commandCode='Εξαγωγή λίστας Εγγραφών Νομοθετημάτων';
	$commandShortName='Λίστα εγγραφών Νομοθετημάτων';
	...
	$columns = array(
		...
		,'C'=>array('Ονομα', 30)
		,'D'=>array('Επώνυμο', 30)
		,'E'=>array('Αρ.Μητρώου', 30)
		,'F'=>array('Τοποθεσία', 25)
		,'G'=>array('Είσοδος', 20)
		,'H'=>array('Εξοδος', 20)
		,'I'=>array('Συν.Χρόνος', 20)
	);
	...
	{	
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A'.$line, $dr["event_id"]);
		
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$line, $dr["tag_id_dec"]);
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$line, $dr["firstname"]);
		$objPHPExcel->getActiveSheet()->setCellValue('D'.$line, $dr['surname']);
		$objPHPExcel->getActiveSheet()->setCellValue('E'.$line, $dr['pnumber']);
		$objPHPExcel->getActiveSheet()->setCellValue('F'.$line, $dr['location_name']);
		
		$objPHPExcel->getActiveSheet()->setCellValue('G'.$line, $dr['event_date']);
		$objPHPExcel->getActiveSheet()->setCellValue('H'.$line, $dr['exit_date']);
		$objPHPExcel->getActiveSheet()->setCellValue('I'.$line, $elapsed);
		$line++;
	}
