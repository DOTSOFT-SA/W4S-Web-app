...
$BaseUrl = "index.php?com=conditions";
$command=array();
...
if( $auth->UserType == "Administrator" )
{
	...
	{	
		if($_REQUEST["Command"] == "SAVE")
		{
			...
			{
				$PrimaryKeys["condition_id"] = intval($_GET["item"]);
			...
			$Collector["condition_name"] = $_POST["condition_name"];
			$QuotFields["condition_name"] = true;
					
			$Collector["description"] =  $_POST["description"]; 
			$QuotFields["description"] = true;
			
			$db->ExecuteUpdater("conditions",$PrimaryKeys,$Collector,$QuotFields);
			...
			{
				$error=0;
				//Δεν μπορώ να σβήσω συνθήκη αν έχω καταχωρημένα maptypes κάτω από αυτό
				$result = $db->sql_query('SELECT * FROM conditions WHERE condition_id='.$item);
				...
				if($error==0) {	
					...
					$filter="";
					$db->sql_query("DELETE FROM conditions WHERE condition_id=" . $item.$filter);
...
if(isset($_GET["item"])) {
	...
	$query="SELECT * FROM conditions WHERE condition_id=".$_GET['item'].$filter;
	$dr_e = $db->RowSelectorQuery($query);
	if (!isset($dr_e["condition_id"]) && intval($_GET["item"])> 0) {
		$messages->addMessage("NOT FOUND!!!");
		Redirect("index.php?com=conditions");
	}
			...
			<section class="card">
				...
				<div class="card-body">
					<div class="form-horizontal form-bordered" method="get">
						...
						<div class="form-group row">
							<label class="col-lg-3 control-label text-lg-right pt-2" for="condition_name">Ονομασία</label>
							<div class="col-lg-6">
								<input type="text" class="form-control" id="condition_name" name="condition_name" value="<?=(isset($dr_e["condition_name"]) ? $dr_e["condition_name"]:'')?>">
							</div>
						</div>
					...
					<div class="row-fluid" style="margin-top:20px;">
						...
						<? } ?>
						<a href="index.php?com=conditions"><button type="button" class="btn btn-primary">Επιστροφή</button></a>
					</div>
				</div>
			</section>
		</div>
	...
	<script>
		...
		//var user_name = $('#user_name').val();
			if ( condition_name.length >= 2  ){ //&& user_name.length >= 5 && user_password.length >= 5
			...
			<div class="col">
				<section class="card">
					<header class="card-header">
						...
						<h2 class="card-title">Συνθήκες</h2>
					</header>
					<div class="card-body">
						...
						<table class="table table-responsive-lg  table-bordered table-striped mb-0" id="datatable-filters">
							...
							<tbody>
								<?	
									...
									$filter="";
									$query = "SELECT * FROM conditions WHERE 1=1 ".$filter."  ";
									...
									while ($dr = $db->sql_fetchrow($result))
									{
										?>
											<tr>
												...
												<td><?=$dr["condition_id"]?></td>
												<td><?=$dr["is_valid"]?></td>
												<td><?=$dr["condition_name"]?></td>
												<td>
													<a data-toggle="tooltip" data-placement="top" title="Επεξεργασία" style="padding:4px"  href="index.php?com=conditions&Command=edit&item=<?=$dr["condition_id"]?>"><i style="font-size:24px;" class="fas fa-edit"></i></a>
													<? if($auth->UserType == "Administrator") {?>
													<a data-toggle="tooltip" data-placement="top" title="Διαγραφή" href="#" onclick="ConfirmDelete('Επιβεβαίωση διαγραφής','index.php?com=conditions&Command=DELETE&item=<?=$dr["condition_id"]?>');"><span><i style="font-size:24px;" class="fas fa-trash"></i> </a></span></a>
													<? } ?>
												</td>
											</tr>
										<?
						...
						<? if($auth->UserType == "Administrator") { ?>
						<div class="row-fluid" style="margin-top:20px;">
							<a href="index.php?com=conditions&item="><button type="button" class="btn btn-primary">Νέα εγγραφή</button></a>
						</div>
						<? } ?>
					</div>
				</section>
