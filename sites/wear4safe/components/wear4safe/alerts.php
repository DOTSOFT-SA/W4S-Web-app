		...
		<div class="col">
			<section class="card">
				<header class="card-header">
					...
					<h2 class="card-title">Λίστα Alerts</h2>
				</header>
				<div class="card-body">
								...
								while ($dr = $db->sql_fetchrow($result))
								{
									...
									?>
										<tr>
											...
											<td><?=$dr["angle"]?></td>
											<td><?=$dr["speed"]?></td>
											<td><?=$dr["probe"] //$sName['maptype_name']?></td> 
											<td><?=$dr["val"]?></td>
											<td><?=$dr["min"]?></td>
											<td><?=$dr["max"]?></td>
											<td><?=$dr["sos"]?></td>
...
<? } else {
	...
	$filter.=($actionDate2!=''?" AND date(event_date)<='".$actionDate2."'":'');
	//actionDate1] => 2022-12-14
	$query = "SELECT t1.event_id,t1.tag_id,t2.tag_id_dec,t1.employee_id,t2.firstname,t2.surname,t2.pnumber,t3.location_name,t1.event_date ,t1.exit_date 
	FROM events t1 
	INNER JOIN employees t2 ON t1.employee_id=t2.employee_id 
	LEFT JOIN locations t3 ON t1.location_id=t3.location_id 
	WHERE 1=1 ".$filter." ORDER BY event_date DESC LIMIT 2000 ";
	*/
	...
	$filter.=(isset($_REQUEST['map']) && intval($_REQUEST['map'])>0?" AND t3.map_id=".intval($map):"");
	$filter.=($auth->UserType != "Administrator"?' AND t4.organization_id='.$auth->UserRow['organization_id']:"");
	$query = "SELECT t1.*,t2.imei,t4.surname,t4.firstname FROM alerts t1 INNER JOIN sensors t2 ON t1.sensor_id=t2.sensor_id INNER JOIN (SELECT * FROM assignments WHERE return_date IS NULL) t3 ON t2.sensor_id=t3.sensor_id INNER JOIN employees t4 ON t3.employee_id=t4.employee_id ORDER BY t1.alert_id DESC LIMIT 2000" ;
	...
	{	
		...
		$objPHPExcel->getActiveSheet()->setCellValue('K'.$line, $dr['min']);
		$objPHPExcel->getActiveSheet()->setCellValue('L'.$line, $dr['max']);
		$objPHPExcel->getActiveSheet()->setCellValue('M'.$line, $dr['sos']);
		$line++;
	}
