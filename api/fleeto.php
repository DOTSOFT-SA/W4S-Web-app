...
$db = new sql_db($host, $dbuser, $dbpass, $database, false);	
	...
	for($i=0;$i<sizeof($data);$i++){
		if(intval($data[$i]['val'])==0){
			$updateQuery = "UPDATE sensors SET lastupdate='".$data[$i]['dt_server']."' WHERE sensor_id='".$data[$i]['sensor_id']."'";
			$message = $data[$i]['dt_server'];
			$results = $db->sql_query($updateQuery);
		} else {
			$updateQuery = "UPDATE sensors SET lastupdate='".$data[$i]['dt_server']."' WHERE sensor_id='".$data[$i]['sensor_id']."'";
			$resultUpdate = $db->sql_query($updateQuery);
			$insertQuery="INSERT INTO alerts(sensor_id, `dt_server`, `dt_tracker`, `lat`, `lng`, `altitude`, `angle`, `speed`, `probe`, `val`, `sos`) VALUES ('".$data[$i]['sensor_id']."','".$data[$i]['dt_server']."','".$data[$i]['dt_tracker']."','".$data[$i]['lat']."','".$data[$i]['lng']."','".$data[$i]['altitude']."','".$data[$i]['angle']."','".$data[$i]['speed']."','".$data[$i]['probe']."','".$data[$i]['val']."','".$data[$i]['sosval']."')";
...
echo '{"status":"'.$message.$insertQuery.'"}';
